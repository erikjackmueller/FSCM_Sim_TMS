...
start = time.time()
# Q = SCSM_jacobi_iter_cupy(tc, areas, n_v, b_im, tol=0.1, n_iter=50, omega=omega, sig_in=sigma_in, sig_out=sigma_out,
#                           verbose=True)
Q = q_jac_vec(tc, areas, n_v, b_im, sig_in=sigma_in, sig_out=sigma_out)
end = time.time()
t = t_format(end - start)
...
# e_field_midlayer_m1s1_pmd_fscm.hdf5
with h5py.File('fscm_jacobi_test_midlayer', 'w') as f:
